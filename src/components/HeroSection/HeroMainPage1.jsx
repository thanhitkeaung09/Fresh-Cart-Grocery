...
const MyComponent = ({data}) => {
    ...
    <div className={'grid grid-cols-8 w-[90%] mx-auto '} >
            <div className="md:col-span-4 col-span-8 flex flex-col  justify-center">
                <div className="">
                    <span
                        className="bg-[#ffc107] text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">Opening Sale Discount
                            {
                               data ? data.map(item => {
                                   return (
                                       // console.log(item.discount)
                                       <span className={'ml-2'} key={item.id}>{item.discount}</span>
                                   )
                                }):console.log("data ma par lota")
                            }
                    %</span>
